...
def restaurant_list_flex(restaurants: list) -> FlexSendMessage:
    ...
    return FlexSendMessage(alt_text='📋 近期推薦餐廳', contents=CarouselContainer(contents=bubbles))
...
def restaurant_detail_flex(restaurant: dict, user_has_liked: bool, comments: list) -> FlexSendMessage:
    ...
    if comments:
        ...
        comment_contents.append(TextComponent(text=f"💬 評論（{comment_count}則）", size='sm', weight='bold', color='#333333'))
        for c in comments[:3]:
            comment_contents.append(
                TextComponent(text=f"• {c['content'][:30]}{'...' if len(c['content'])>30 else ''}", size='xs', wrap=True, color='#555555')
            )
...
def photos_carousel_flex(restaurant_name: str, photos: list, user_liked_ids: set) -> FlexSendMessage:
    """照片輪播，每張有按讚按鈕，底部提示可輸入「檢舉 X」"""
    bubbles = []
    for i, p in enumerate(photos, 1):
        ...
        if p.get('status') == 'pending_review':
            status_note = [TextComponent(text='⚠️ 此照片待審核中', size='xs', color='#E67E22', margin='sm')]

        bubble = BubbleContainer(
            hero=ImageComponent(url=p['image_url'], size='full', aspect_ratio='4:3', aspect_mode='cover'),
            body=BoxComponent(
                layout='vertical', spacing='sm',
                contents=[
